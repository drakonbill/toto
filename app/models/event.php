...
class models_event extends Model {
    ...
    function searchModel() {
        ...
        $request_member = "SELECT lati_member,long_member FROM member WHERE id_member='" . $_SESSION['id_member'] . "'";
        $result_member = mysql_query($request_member) or die(mysql_error());
        if (mysql_num_rows($result_member) == 1) {
            $data_member = mysql_fetch_array($result_member);
            $data_search->coord_member = $data_member;
        }
        ...
        while ($data_passion = mysql_fetch_array($result_passion))
            $tab_passion[] = $data_passion;
        $data_search->passion_category = $tab_passion;

        $tab_event = array();
        $request_event = "SELECT *, distance('km', ".$data_member['lati_member'].", ".$data_member['long_member'].", latitude_event, longitude_event) AS distance_event FROM event E1 WHERE id_member=12 AND end_date_event > NOW() HAVING distance_event <= 25 UNION
            SELECT *, distance('km', ".$data_member['lati_member'].", ".$data_member['long_member'].", latitude_event, longitude_event) AS distance_event FROM event E1 WHERE confidentiality_event = 0 AND id_member != 12 AND (id_member IN (SELECT id_contact FROM member_contacts WHERE condition_contact = 'accepte' AND id_member=12) OR EXISTS (SELECT * FROM event_participant WHERE id_member=12 AND status IN ('accepte','accepte_invite','invite') AND id_event=E1.id_event)) AND end_date_event > NOW() HAVING distance_event <= 25 UNION
            SELECT *, distance('km', ".$data_member['lati_member'].", ".$data_member['long_member'].", latitude_event, longitude_event) AS distance_event FROM event E1 WHERE confidentiality_event = 1 AND id_member != 12 AND end_date_event > NOW() HAVING distance_event <= 25 UNION
            SELECT *, distance('km', ".$data_member['lati_member'].", ".$data_member['long_member'].", latitude_event, longitude_event) AS distance_event FROM event E1 WHERE confidentiality_event = 2 AND id_member != 12 AND (EXISTS(SELECT id_member FROM event_confidentiality WHERE event_confidentiality.id_event=E1.id_event AND event_confidentiality.id_member=12) OR EXISTS (SELECT * FROM event_participant WHERE id_member=12 AND status IN ('accepte','accepte_invite','invite') AND id_event=E1.id_event)) AND end_date_event > NOW() HAVING distance_event <= 25
            ORDER BY distance('km', ".$data_member['lati_member'].", ".$data_member['long_member'].", latitude_event, longitude_event)";
