...
class models_event extends Model {
    ...
    function searchModel() {
        ...
        $tab_event = array();
        $request_event = "SELECT *, distance('km', ".$data_member['lati_member'].", ".$data_member['long_member'].", latitude_event, longitude_event) AS distance_event FROM event WHERE id_member=12 AND end_date_event > NOW() UNION
        SELECT *, distance('km', ".$data_member['lati_member'].", ".$data_member['long_member'].", latitude_event, longitude_event) AS distance_event FROM event WHERE confidentiality_event = 0 AND id_member != 12 AND id_member IN (SELECT id_contact FROM member_contacts WHERE condition_contact = 'accepte' AND id_member=12) AND end_date_event > NOW() UNION
        SELECT *, distance('km', ".$data_member['lati_member'].", ".$data_member['long_member'].", latitude_event, longitude_event) AS distance_event FROM event WHERE confidentiality_event = 1 AND id_member != 12 AND end_date_event > NOW() UNION
        SELECT *, distance('km', ".$data_member['lati_member'].", ".$data_member['long_member'].", latitude_event, longitude_event) AS distance_event FROM event WHERE confidentiality_event = 2 AND id_member != 12 AND id_member IN (SELECT id_member FROM event_confidentiality WHERE event_confidentiality.id_event=event.id_event) AND end_date_event > NOW()
